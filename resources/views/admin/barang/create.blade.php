...
<div class="max-w-lg mx-auto py-8">
    ...
    <form action="{{ route('barang.store') }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <textarea name="deskripsi" class="w-full border rounded px-3 py-2" required>{{ old('deskripsi') }}</textarea>
        </div>
        <div>
            <label class="block mb-1">Stok</label>
            <input type="number" name="stok" class="w-full border rounded px-3 py-2" required min="0" value="{{ old('stok') }}">
        </div>
        <div>
